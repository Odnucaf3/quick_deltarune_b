...
#-------------------------------------------------------------------------------
@export_category("Party & Inventory")
@export var ally_node_array: Array[Fighter_Node]
var fighter_index: int
...
func _ready() -> void:
	...
	Set_Idiome()
	NormalMotion()
	Animation_StateMachine(ally_node_array[0].animation_tree, state_machine_layer_1, "Idle")
#-------------------------------------------------------------------------------
...
func Player_Movement():
	...
	if(abs(input_dir.y) < dead_zone):
		input_dir.y = 0
	#-------------------------------------------------------------------------------
	if(ally_node_array[0].is_Moving):
		#-------------------------------------------------------------------------------
		if(input_dir == Vector2.ZERO):
			Animation_StateMachine(ally_node_array[0].animation_tree, state_machine_layer_1, "Idle")
			ally_node_array[0].is_Moving = false
		...
		#-------------------------------------------------------------------------------
			...
			if(is_Running):
				...
				#-------------------------------------------------------------------------------
				if(!_run_flag):
					Animation_StateMachine(ally_node_array[0].animation_tree, state_machine_layer_1, "Walk")
					is_Running = false
				#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
			else:
				...
				#-------------------------------------------------------------------------------
				if(_run_flag):
					Animation_StateMachine(ally_node_array[0].animation_tree, state_machine_layer_1, "Run")
					is_Running = true
				#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
			if(ally_node_array[0].is_Facing_Left):
				if(input_dir_normal.x > 0):
					Face_Left(ally_node_array[0], false)
					return
				#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
			else:
				if(input_dir_normal.x < 0):
					Face_Left(ally_node_array[0], true)
					return
				#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
		...
		if(input_dir != Vector2.ZERO):
			...
			#-------------------------------------------------------------------------------
				...
				is_Running = false
			#-------------------------------------------------------------------------------
			ally_node_array[0].is_Moving = true
			return
		#-------------------------------------------------------------------------------
		else:
...
func PauseMenu_Open():
	...
	for _i in ally_node_array.size():
		...
		ally_node_array[_i].fighter_button = _party_button
...
func PauseMenu_Close():
	...
	#-------------------------------------------------------------------------------
	for _i in ally_node_array.size():
		ally_node_array[_i].fighter_button.queue_free()
	#-------------------------------------------------------------------------------
	Pause_Off()
#-------------------------------------------------------------------------------
func Pause_Off():
...
func Create_Fighter_Button(_fighter_node:Fighter_Node) -> Fighter_Button:
	...
	#-------------------------------------------------------------------------------
	_party_button.face.texture = _fighter_node.fighter_serializable.fighter_resource.face
...
func Pause_Menu_Set():
	...
	singleton.Button_Array_Set_Vertical_Navigation(_button_array)
	#-------------------------------------------------------------------------------
	var _fighter_button_array: Array[Button]
	#-------------------------------------------------------------------------------
	for _i in ally_node_array.size():
		_fighter_button_array.append(ally_node_array[_i].fighter_button)
	#-------------------------------------------------------------------------------
	singleton.Button_Array_Set_Vertical_Navigation(_fighter_button_array)
#-------------------------------------------------------------------------------
func Pause_Menu_Skill_Button_Submit():
	...
	for _i in ally_node_array.size():
		...
		#-------------------------------------------------------------------------------
		singleton.Set_Button(ally_node_array[_i].fighter_button, _selected, _submit, _cancel)
	#-------------------------------------------------------------------------------
	Disable_Item_Button_0(pause_menu_button_skill)
	singleton.Move_to_Button_by_Submit(ally_node_array[0].fighter_button)
#-------------------------------------------------------------------------------
func Pause_Menu_Skill_Fighter_Button_Submit(_index:int):
	fighter_index = _index
	Pause_Skill_Menu_Set()
	pause_menu.hide()
	skill_menu.show()
#-------------------------------------------------------------------------------
...
func Pause_Menu_Equip_Button_Submit():
	...
	singleton.Move_to_Button_by_Submit(ally_node_array[0].fighter_button)
#-------------------------------------------------------------------------------
func Pause_Menu_Equip_Fighter_Button_Submit(_index:int):
	fighter_index = _index
	pause_menu.hide()
	Pause_Equip_Menu_Set()
	equip_menu.show()
#-------------------------------------------------------------------------------
...
func Pause_Menu_Statistics_Button_Submit():
	...
	singleton.Move_to_Button_by_Submit(ally_node_array[0].fighter_button)
#-------------------------------------------------------------------------------
func Pause_Menu_Statistics_Fighter_Button_Submit(_index:int):
	fighter_index = _index
	pause_menu.hide()
	statistics_menu.show()
	singleton.Move_to_Button_by_Submit(statistics_menu_button_0)
	Pause_Statistics_Menu_Set()
...
func Pause_Menu_Status_Button_Submit():
	...
	singleton.Move_to_Button_by_Submit(ally_node_array[0].fighter_button)
#-------------------------------------------------------------------------------
func Pause_Menu_Status_Fighter_Button_Submit(_index:int):
	fighter_index = _index
	pause_menu.hide()
	status_menu.show()
	Pause_Status_Menu_Set()
...
func Pause_Skill_Menu_Set():
	...
	if(_skill_serializable_array.size() > 0):
		...
		for _i in _skill_serializable_array.size():
			...
			var _cancel: Callable = func(): Pause_Skill_Menu_Main_Button_Cancel()
			#-------------------------------------------------------------------------------
...
func Create_Skill_Button(_skill_serializable:Action_Serializable):
	...
	return _button
#-------------------------------------------------------------------------------
func Pause_Skill_Menu_Main_Button_Cancel():
	pause_menu.show()
	skill_menu.hide()
	singleton.Destroy_Button_Array(skill_menu_button_array)
	singleton.Move_to_Button_by_Cancel(ally_node_array[fighter_index].fighter_button)
#-------------------------------------------------------------------------------
...
func Pause_Item_Menu_Set():
	...
	#-------------------------------------------------------------------------------
	if(item_consumable_serializable_array.size() > 0):
		#-------------------------------------------------------------------------------
		for _i in item_consumable_serializable_array.size():
			var _hold: int = item_consumable_serializable_array[_i].hold
			var _cooldown: int = 0
			#-------------------------------------------------------------------------------
			var _consumable_button: Button = Create_ConsumableItem_Button(item_consumable_serializable_array[_i], _hold, _cooldown)
			#-------------------------------------------------------------------------------
			...
			#-------------------------------------------------------------------------------
			var _all_button: Button = Create_ConsumableItem_Button(item_consumable_serializable_array[_i], _hold, _cooldown)
			#-------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------------
	if(item_equip_serializable_array.size() > 0):
		#-------------------------------------------------------------------------------
		for _i in item_equip_serializable_array.size():
			var _equip_button: Button = Create_EquipItem_Button(item_equip_serializable_array[_i])
			#-------------------------------------------------------------------------------
			...
			#-------------------------------------------------------------------------------
			var _all_button: Button = Create_EquipItem_Button(item_equip_serializable_array[_i])
			#-------------------------------------------------------------------------------
			var _all_select_1: Callable = func():
				Set_Item_Equip_Information(item_equip_serializable_array[_i])
				singleton.Common_Selected()
	...
	#-------------------------------------------------------------------------------
	if(item_key_serializable_array.size() > 0):
		#-------------------------------------------------------------------------------
		for _i in item_key_serializable_array.size():
			var _key_button: Button = Create_KeyItem_Button(item_key_serializable_array[_i])
			#-------------------------------------------------------------------------------
			...
			#-------------------------------------------------------------------------------
			var _all_button: Button = Create_KeyItem_Button(item_key_serializable_array[_i])
			#-------------------------------------------------------------------------------
			var _all_select_1: Callable = func():
				Set_Item_Key_Information(item_key_serializable_array[_i])
				singleton.Common_Selected()
...
func Set_Item_Equip_Information(_equip_serializable:Equip_Serializable):
	...
	#-------------------------------------------------------------------------------
	item_menu_equip_information_level_value.text = str(_equip_resource.level_required)
...
func Set_User_Equip_Information(_equip_serializable:Equip_Serializable):
	...
	if(_equip_serializable.equip_resource != null):
		var _equip_resource: Equip_Resource = _equip_serializable.equip_resource
		#-------------------------------------------------------------------------------
		equip_menu_information_level_value.text = str(_equip_resource.level_required)
...
func Set_Equip_Element_4_Stats(_name_label:Label, _element_type:Action_Resource.ELEMENT, _value_label:Label, _element:Vector4i):
	...
	if(_element.x != 0):
		_name_label.text += String_With_Asterisco_and_2_Points(tr("power_text")+_element_name)+"\n"
		_value_label.text += Get_Number_with_Sign(_element.x)+"%"+"\n"
	#-------------------------------------------------------------------------------
	if(_element.y != 0):
		_name_label.text += String_With_Asterisco_and_2_Points(tr("absortion_text")+_element_name)+"\n"
		_value_label.text += Get_Number_with_Sign(_element.y)+"%"+"\n"
	#-------------------------------------------------------------------------------
	if(_element.z != 0):
		_name_label.text += String_With_Asterisco_and_2_Points(tr("affinity_text")+_element_name)+"\n"
		_value_label.text += Get_Number_with_Sign(_element.z)+"%"+"\n"
	#-------------------------------------------------------------------------------
	if(_element.w != 0):
		_name_label.text += String_With_Asterisco_and_2_Points(tr("repulsion_text")+_element_name)+"\n"
...
func Set_Equip_Status_Resistance_Rate(_name_label:Label, _value_label:Label, _status_dictionary:Dictionary[StringName, int]):
	...
	for _i in _status_dictionary.size():
		...
		var _name: StringName = " ("+tr("name_"+_key)+")"
		_name_label.text += String_With_Asterisco_and_2_Points(tr("resistance_text")+_name)+"\n"
...
func Pause_Equip_Menu_Set():
	...
	if(_equip_serializable_array.size() >0):
		...
		for _i in _equip_serializable_array.size():
			...
			var _selected_1: Callable = func():
				...
				singleton.Common_Selected()
			#-------------------------------------------------------------------------------
			var _submit_1: Callable = func():Pause_Equip_Menu_Equip_Slot_Submit(_equip_serializable_array, _i)
			var _cancel_1: Callable = func():Pause_Equip_Menu_Main_Button_Cancel()
			#-------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------------
		...
		var _selected: Callable = func():singleton.Common_Selected()
		var _submit: Callable = func():pass
		var _cancel: Callable = func():Pause_Equip_Menu_Main_Button_Cancel()
		#-------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
func Pause_Equip_Menu_Equip_Slot_Submit(_equip_Serializable_array:Array[Equip_Serializable], _equip_index:int):
	equip_menu.hide()
	item_menu.show()
	#-------------------------------------------------------------------------------
	...
	var _selected_0: Callable = func():
		...
		singleton.Common_Selected()
	#-------------------------------------------------------------------------------
	var _submit_0: Callable = func():Pause_Equip_Menu_Equip_Slot_Equip_Empty_Submit(_equip_index)
	...
	#-------------------------------------------------------------------------------
	var _current_Fighter: Fighter_Serializable = ally_node_array[fighter_index].fighter_serializable
	var _current_equip_slot: Equip_Serializable = _equip_Serializable_array[_equip_index]
	#-------------------------------------------------------------------------------
	for _i in item_equip_serializable_array.size():
		if(Is_Weapon_Avalible_to_be_Equipable(item_equip_serializable_array[_i].equip_resource, _current_equip_slot, _current_Fighter.fighter_resource.myFIGHTER_CLASS)):
			...
			var _selected_1: Callable = func():
				...
				singleton.Common_Selected()
			#-------------------------------------------------------------------------------
			var _submit_1: Callable = func(): Pause_Equip_Menu_Equip_Slot_Equip_Item_Submit(_equip_index, item_equip_serializable_array[_i])
...
func Is_Weapon_Avalible_to_be_Equipable(_equip_resource:Equip_Resource, _equip_serializable:Equip_Serializable, _myFIGHTER_CLASS:Fighter_Resource.FIGHTER_CLASS) -> bool:
	...
	#-------------------------------------------------------------------------------
		...
		return false
#-------------------------------------------------------------------------------
func Pause_Equip_Menu_Equip_Slot_Equip_Item_Submit(_equip_index:int, _equip_serializable:Equip_Serializable):
	item_menu.hide()
	equip_menu.show()
	singleton.Destroy_Button_Array(item_menu_equip_button_array)
	#-------------------------------------------------------------------------------
	var _equip_slot: Equip_Serializable = ally_node_array[fighter_index].fighter_serializable.equip_serializable_array[_equip_index]
	#-------------------------------------------------------------------------------
	...
	Equip_Item_to_Fighter(_equip_index, _equip_serializable)
	#-------------------------------------------------------------------------------
	singleton.Move_to_Button_by_Equip(equip_menu_button_array[_equip_index])
#-------------------------------------------------------------------------------
func Pause_Equip_Menu_Equip_Slot_Equip_Empty_Submit(_equip_index:int):
	item_menu.hide()
	equip_menu.show()
	singleton.Destroy_Button_Array(item_menu_equip_button_array)
	#-------------------------------------------------------------------------------
	var _current_equip_slot: Equip_Serializable = ally_node_array[fighter_index].fighter_serializable.equip_serializable_array[_equip_index]
	#-------------------------------------------------------------------------------
	if(_current_equip_slot.equip_resource != null):
		Add_Equip_Item_To_Inventory(_current_equip_slot.equip_resource, 1)
	#-------------------------------------------------------------------------------
	Unequip_Item_to_Fighter(_equip_index)
	#-------------------------------------------------------------------------------
	singleton.Move_to_Button_by_Unequip(equip_menu_button_array[_equip_index])
#-------------------------------------------------------------------------------
func Equip_Item_to_Fighter(_equip_index:int, _equip_serializable:Equip_Serializable):
	var _equip_slot: Equip_Serializable = ally_node_array[fighter_index].fighter_serializable.equip_serializable_array[_equip_index]
...
#-------------------------------------------------------------------------------
func Unequip_Item_to_Fighter(_equip_index:int):
	var _equip_slot: Equip_Serializable = ally_node_array[fighter_index].fighter_serializable.equip_serializable_array[_equip_index]
...
#-------------------------------------------------------------------------------
func Pause_Equip_Menu_Main_Button_Cancel():
	singleton.Destroy_Button_Array(equip_menu_button_array)
	pause_menu.show()
	equip_menu.hide()
	Fighter_Button_Set_HP(ally_node_array[fighter_index].fighter_button, ally_node_array[fighter_index].fighter_serializable)
	var _button: Button = ally_node_array[fighter_index].fighter_button as Button
...
#-------------------------------------------------------------------------------
func Pause_Statistics_Menu_Set():
	#-------------------------------------------------------------------------------
	...
	var _selected: Callable = func():singleton.Common_Selected()
	var _submit: Callable = func():pass
	var _cancel: Callable = func():Pause_Statistics_Menu_Main_Button_Cancel()
	#-------------------------------------------------------------------------------
	singleton.Set_Button_WS_Up_Down(statistics_menu_button_0, _selected, _submit, _cancel, _w, _s)
	#-------------------------------------------------------------------------------
	var _fighter_serializable: Fighter_Serializable = ally_node_array[fighter_index].fighter_serializable
	#-------------------------------------------------------------------------------
	statistics_menu_information_fighter_face.texture = _fighter_serializable.fighter_resource.face
	#-------------------------------------------------------------------------------
	...
	statistics_menu_information_fighter_hp_value.text = str(_max_hp)+" / "+str(_max_hp)+" PV"
...
#-------------------------------------------------------------------------------
func Pause_Statistics_Menu_Main_Button_Cancel():
	pause_menu.show()
	statistics_menu.hide()
	var _button: Button = ally_node_array[fighter_index].fighter_button as Button
	singleton.Move_to_Button_by_Cancel(_button)
...
func Create_StatusEffect_Serializable_Button(_status_serializable: Status_Serializable) -> Button:
	...
	return _button
#-------------------------------------------------------------------------------
func Pause_Status_Menu_Status_Button_Cancel():
...
func Fighter_Button_Set_HP(_fighter_button:Fighter_Button, _fighter_Serializable:Fighter_Serializable):
	...
	_fighter_button.hp_label.text = str(_max_hp)+" / "+str(_max_hp)+" PV"
...
func Set_Idiome():
	...
	#-------------------------------------------------------------------------------
	pause_menu_button_options.text = String_With_2_Spaces(tr("pause_menu_button_options"))
	...
	#-------------------------------------------------------------------------------
	for _i in ally_node_array.size():
		Fighter_Button_Set_Information_and_Idiome(ally_node_array[_i].fighter_button, ally_node_array[_i].fighter_serializable)
	#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
func Face_Left(_user:Fighter_Node, _b:bool):
	#-------------------------------------------------------------------------------
	if(_b):
...
func Debug_Information() -> void:
	...
	_s += "Current Turn ID: " + str(fighter_index)+"\n"
