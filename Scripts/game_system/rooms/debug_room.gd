...
func NPC_1_Talk(_imteractable_script:Interactable_Script, _character_node:Character_Node, _fighter_resource:Fighter_Resource, _fighter_prefab:PackedScene):
	singleton.game_system.Dialogue_Enter_and_Open()
	#-------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------------
	if(_index < 0):
		await singleton.game_system.Dialogue_with_Face_with_Options(_character_node.character_resource, "* Querés que te acompañe?", ["Si", "No"])
		#-------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------------
		...
		await singleton.game_system.Dialogue_with_Face_with_Options(_character_node.character_resource, "* Querés que me vaya del grupo?", ["Si", "No"])
		#-------------------------------------------------------------------------------
		match(singleton.game_system.dialogue_option_index):
			0:
				#-------------------------------------------------------------------------------
...
func NPC_3_Talk():
	...
	singleton.game_system.Dialogue_Enter_and_Open()
	#-------------------------------------------------------------------------------
	await singleton.game_system.Dialogue_with_Face_with_Options(npc_3_character.character_resource, "* Hola Che. Vamos a pelear!", ["Comprar", "Hablar", "Irse"])
	#-------------------------------------------------------------------------------
	...
	match(singleton.game_system.dialogue_option_index):
		0:
			...
			singleton.game_system.Dialogue_Close()
...
func NPC_4_Talk(_array_fighter_array:Array[Fighter_Node]):
	...
	singleton.game_system.Dialogue_Enter_and_Open()
	#-------------------------------------------------------------------------------
	await singleton.game_system.Dialogue_with_Face_with_Options(_character_resource, "* Hola Che. Vamos a pelear!", ["Pelear", "Irse"])
	#-------------------------------------------------------------------------------
	match(singleton.game_system.dialogue_option_index):
		0:
			#-------------------------------------------------------------------------------
